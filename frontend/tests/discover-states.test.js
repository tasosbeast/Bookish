...
test('Discover retains results through a filter refresh and failed retry path', { timeout: 60000 }, async t => {
  ...
  globalThis.fetch = input => {
    ...
    if (url.pathname === '/api/recommendations/top-picks') return Promise.resolve(response({ data: [], meta: { personalized: false, ratedBooks: 0, minimumRatings: 3 } }));
    if (url.pathname === '/api/releases') return Promise.resolve(response({ newReleases: [], upcoming: [] }));
