...
export default function BookDetails() {
  ...
  return <div className="container details-page"><Link className="back-link" to="/">← Back to discover</Link><ErrorNotice error={book.error} retry={book.reload} /><section className="book-detail-grid"><div className="detail-cover-wrap"><Cover book={data} className="detail-cover" /><span className="cover-caption">A place on your bookshelf.</span></div><div className="detail-copy"><p className="eyebrow">Between the covers</p><h1>{data.title}</h1><p className="detail-author">by <Link className="author-filter" to={`/?author=${encodeURIComponent(data.author)}`}>{data.author}</Link></p><Rating value={data.averageRating} count={data.ratingsCount} /><Genres genres={data.genres} /><p className="book-description">{data.description || 'There isn’t a description for this book yet.'}</p><dl className="book-facts">{data.publicationYear && <div><dt>Published</dt><dd>{data.publicationYear}</dd></div>}{data.isbn && <div><dt>ISBN</dt><dd>{data.isbn}</dd></div>}</dl>
