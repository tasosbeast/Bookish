...
export default function Discover() {
  ...
  const hasFilters = q || genre || author;
  ...
  return <div className="container discover-page">
    ...
    <section aria-labelledby="discover-heading" className="catalog-section"><div className="section-heading"><div><p className="eyebrow">The bookshelf</p><h2 id="discover-heading">{author ? `Books by “${author}”` : q ? `Results for “${q}”` : 'Discover something good'}</h2></div>{resource.data && <span className="muted small" role="status" aria-live="polite">{resource.loading ? 'Updating results…' : resource.error ? 'Showing previous results.' : `${resource.data.pagination.total} ${resource.data.pagination.total === 1 ? 'book' : 'books'}`}</span>}</div>
      ...
      {initialLoading ? <Loading cards /> : resource.data?.data.length ? <>
        <div className="book-grid">{resource.data.data.map(book => <article className="book-card" key={book.id}><Link to={`/books/${book.id}`} className="cover-link" aria-label={`Read about ${book.title}`}><Cover book={book} /><span className="cover-open"><Icon name="arrow" /></span></Link><div className="book-card-meta"><Rating value={book.averageRating} /><h3><Link to={`/books/${book.id}`}>{book.title}</Link></h3><p><button type="button" className="author-filter" onClick={() => change({ author: book.author })}>{book.author}</button></p><Genres genres={book.genres} onSelect={slug => change({ genre: slug })} /></div></article>)}</div>
      </> : !resource.error && <EmptyState title={emptyPage ? 'No books on this page' : hasFilters ? noResultsTitle : 'The first chapter is still to come'} action={emptyPage ? <button className="button secondary" onClick={() => change({ page: 1 }, true)}>Back to first page</button> : hasFilters && <button className="button secondary" onClick={() => { setSearch(''); setParams({}); }}>Clear filters</button>}>{emptyPage ? 'Return to the first page to explore the books in these results.' : hasFilters ? 'Try a different title, author, or genre. A good read may be just one search away.' : 'There aren’t any books in the catalog yet. When books are added, you’ll find them here.'}</EmptyState>}
