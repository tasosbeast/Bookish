...
export default function MyBooks() {
  ...
  const shelves = useResource(`/user-books?${query}`, 'required');
  ...
  return <div className="container my-books-page"><div className="page-heading"><p className="eyebrow">Your own little library</p><h1>My books<span className="brand-dot">.</span></h1><p className="muted">The stories you’re saving, living in, and looking back on.</p></div><nav className="shelf-tabs" aria-label="Reading status">{[['', 'All books'], ...statuses].map(([value, label]) => <button key={value} aria-current={status === value ? 'page' : undefined} className={status === value ? 'selected' : ''} onClick={() => filter(value)}>{label}</button>)}</nav>{notice && <p className="success-notice" role="status">{notice}</p>}
    {shelves.loading ? <Loading /> : shelves.error ? <ErrorNotice error={shelves.error} retry={shelves.reload} /> : shelves.data.data.length ? <><p className="muted small shelf-count">{shelves.data.pagination.total} {shelves.data.pagination.total === 1 ? 'book' : 'books'} on this shelf</p><div className="shelf-list">{shelves.data.data.map(entry => <article className="shelf-row" key={entry.bookId}><div className="shelf-row-main"><Link to={`/books/${entry.bookId}`} aria-label={`Read about ${entry.book.title}`}><Cover book={entry.book} className="shelf-cover" /></Link><div className="shelf-book-info"><Genres genres={entry.book.genres} /><h2><Link to={`/books/${entry.bookId}`}>{entry.book.title}</Link></h2><p className="muted">{entry.book.author}</p><Rating value={entry.book.averageRating} /></div><div className="shelf-personal"><span className="status-badge">{statuses.find(([value]) => value === entry.status)?.[1]}</span><span className="small muted">Your rating: {entry.userRating ? `${entry.userRating} / 5` : 'Not rated'}</span><button className="text-button" aria-expanded={editing === entry.bookId} onClick={() => setEditing(editing === entry.bookId ? null : entry.bookId)}>{editing === entry.bookId ? 'Close editor' : 'Update reading'}</button></div></div>{editing === entry.bookId && <div className="inline-editor"><ShelfEditor bookId={entry.bookId} onSaved={saved} /></div>}</article>)}</div></> : <EmptyState title={emptyPage ? 'No books on this page' : status ? 'A little room on this shelf' : 'Your reading story starts here'} action={emptyPage ? <button className="button" onClick={() => filter(status)}>Back to first page</button> : <Link className="button" to="/">Discover books</Link>}>{emptyPage ? 'Your books are still here. Return to the first page of this shelf.' : status ? 'No books on this page yet. Discover a book or choose another shelf.' : 'Find something that catches your eye and add it to your books.'}</EmptyState>}
    {!shelves.loading && <Pagination pagination={shelves.data?.pagination} onPage={page => { setEditing(null); setParams({ ...(status && { status }), page }); }} />}
  </div>;
}
